...
const MAIN_NUMBER_SCALE = 0.9; 

const PARTICLES_PER_ZONE = 120;
const EDGE_TAPER = 0.7;

// INCREASED RANDOMNESS: This spreads particles out from the exact font points
const GRID_RANDOMNESS = 12; 
const BASE_PARTICLE_SCALE = 1.4; 
const BEAT_SCALE_BOOST = 0.35;    
const BEAT_JOLT_STRENGTH = 12;   
const JOLT_DECAY = 0.1;         

const SEPARATION_RADIUS = 35; // Slightly wider to prevent clumping
...
function draw() {
  ...
  for (let z = 0; z < 4; z++) {
    ...
    // 2. PARTICLE LOGIC
    // Increased step size slightly to make the "points" more varied
    ...
    if (!initialPositionsSet && pts.length > 0) {
      for (let i = 0; i < zoneParticles[z].length; i++) {
        ...
        // Add unique random offset immediately so they don't start in a grid
    ...
    for (let i = 0; i < zoneParticles[z].length; i++) {
      ...
      if (pts.length > 0) { 
        ...
        // Per-particle noise for floating randomness
...
function drawLayout(time, dateDayText, cityCountryText) {
  ...
  for (let i = 0; i < 4; i++) {
    let startX = i * zoneW;
    fill(255); noStroke(); textFont(footerFont); textAlign(LEFT, BOTTOM); textSize(60);
    text(time, startX + 60, height - 20);
    push();
    textFont(sidebarFont); fill('#BBB6C3'); noStroke();
    translate(startX + zoneW - 70, 30); rotate(-HALF_PI); textAlign(RIGHT, CENTER); textSize(20);
    text(cityCountryText, 0, 0);
    pop();
    push();
    textFont(sidebarFont); fill('#BBB6C3'); translate(startX + zoneW - 70, height - 25);
    rotate(-HALF_PI); textAlign(LEFT, CENTER); textSize(20); text(dateDayText, 0, 0);
    pop();
    if (i < 3) { 
...
class Particle {
  constructor(minX, maxX, zoneIndex) {
    ...
    this.baseSize = random(20, 34) * BASE_PARTICLE_SCALE; 
